...
dnf install -y rpm-build dnf-plugins-core sed findutils

# ── Download Fedora 42 mesa SRPM ──────────────────────────────────────────────
# Targets mesa 26.1.x; if Fedora 42 ships a newer stable, that will be pulled.
cd ~/build
...
# Pattern covers both `-Dvideo-codecs=` (empty) and `-Dvideo-codecs=somevalue`.
